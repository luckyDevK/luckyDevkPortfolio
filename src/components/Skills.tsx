        ...
        <div className="text-center">
          ...
          <div className="flex flex-wrap justify-center gap-3">
            {technologies.map((tech, index) => (
              <div
                key={index}
                className="flex items-center gap-2 px-4 py-2 bg-dark-800 border border-dark-700 rounded-full hover:border-go-cyan transition-colors cursor-default"
              >
                {typeof tech.logo === 'string' && !tech.logo.includes('.svg') ? (
                  <span>{tech.logo}</span>
                ) : (
                  <img src={tech.logo as string} alt={tech.name} className="w-5 h-5" />
                )}
